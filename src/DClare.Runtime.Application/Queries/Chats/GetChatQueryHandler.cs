...
/// <summary>
/// Represents the service used to handle <see cref="GetChatQuery"/> instances
/// </summary>
...
public class GetChatQueryHandler(IHttpContextAccessor httpContextAccessor, IChatManager chatManager)
    : IQueryHandler<GetChatQuery, Chat>
{

    /// <inheritdoc/>
    public virtual async Task<IOperationResult<Chat>> HandleAsync(GetChatQuery query, CancellationToken cancellationToken = default)
    {
        ...
        return this.Ok(await chatManager.GetAsync(query.Id, userId, query.Agent, cancellationToken).ConfigureAwait(false));
    }
