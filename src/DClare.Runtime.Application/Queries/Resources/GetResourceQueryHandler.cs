...
public sealed class GetResourceQueryHandler<TResource>(IResourceRepository repository)
    : IQueryHandler<GetResourceQuery<TResource>, TResource>
     where TResource : class, IResource, new()
{

    /// <inheritdoc/>
    public async Task<IOperationResult<TResource>> HandleAsync(GetResourceQuery<TResource> query, CancellationToken cancellationToken)
    {
