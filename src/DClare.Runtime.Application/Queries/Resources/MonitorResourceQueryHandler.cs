...
public sealed class MonitorResourceQueryHandler<TResource>(IResourceRepository repository)
    : IQueryHandler<MonitorResourceQuery<TResource>, IAsyncEnumerable<IResourceWatchEvent<TResource>>>
    where TResource : class, IResource, new()
{

    /// <inheritdoc/>
    public async Task<IOperationResult<IAsyncEnumerable<IResourceWatchEvent<TResource>>>> HandleAsync(MonitorResourceQuery<TResource> query, CancellationToken cancellationToken)
    {
