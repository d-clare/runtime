...
/// <summary>
/// Represents the default implementation of the <see cref="IChatManager"/> interface
/// </summary>
/// <param name="cache">The service used to cache <see cref="Chat"/>s</param>
/// <param name="jsonSerializer">The service used to serialize/deserialize data to/from JSON</param>
public class ChatManager(IDistributedCache cache, IJsonSerializer jsonSerializer)
    : IChatManager
{

    /// <summary>
    /// Gets the service used to cache <see cref="Chat"/>s
    /// </summary>
    ...
    /// <inheritdoc/>
    public virtual async Task AddOrUpdateAsync(Chat chat, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc/>
    public virtual async Task<Chat?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        return string.IsNullOrWhiteSpace(json) ? null : JsonSerializer.Deserialize<Chat>(json);
    }

    /// <inheritdoc/>
    public virtual Task<Chat?> GetAsync(string id, string userId, string agentName, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(id);
        ArgumentException.ThrowIfNullOrWhiteSpace(userId);
        ArgumentException.ThrowIfNullOrWhiteSpace(agentName);
        return GetAsync(Chat.BuildKey(id, userId, agentName), cancellationToken);
    }

    /// <inheritdoc/>
    public virtual async IAsyncEnumerable<Chat> ListAsync(string userId, string? agentName = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(userId);
    ...
    public virtual async Task RenameAsync(string key, string? name, CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = BuildChatCacheKey(key);
        var json = await Cache.GetStringAsync(cacheKey, cancellationToken).ConfigureAwait(false);
        var chat = string.IsNullOrWhiteSpace(json) ? null : JsonSerializer.Deserialize<Chat>(json);
        if (chat == null) return;
        chat.Name = name;
